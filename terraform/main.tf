...
resource "google_cloud_run_service" "backend" {
    ...
    template{
        spec{
            ...
            containers{
                image = "europe-west1-docker.pkg.dev/ceri-m1-ecommerce-2022/whitehorse/backend:0.0.2"
                env{
                    name="USER"
                    value_from{
                        secret_key_ref{
...
resource "google_cloud_run_service" "frontend" {
  ...
  template {
    spec {
      service_account_name = "terraform-whitehorse@ceri-m1-ecommerce-2022.iam.gserviceaccount.com"
      containers {
        image = "europe-west1-docker.pkg.dev/ceri-m1-ecommerce-2022/whitehorse/frontend:0.0.2"
        env {
          name = "BACKEND_URL"
